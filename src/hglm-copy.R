...
if (!is.null(startval)) {
	...
	init.sig.u <- as.numeric(startval[(ncol(x) + sum(RandC) + 1):(ncol(x) + sum(RandC) + k)])
...
while (iter <= maxit) {
	...
	#}
    g.mme <- hglm:::GLM.MME(Augy = Augy, AugXZ = AugXZ, starting.delta = c(b.hat, v.i), tau = tau, phi = phi, 
                     n.fixed = ncol(x), n.random = nRand[k], weights.sqrt = w, prior.weights, family, 
                     rand.family, maxit, sparse = sparse, off = off, tol = tol, colidx = colidx,
					 HL.correction = HL.correction)
    b.hat <- g.mme$b.hat
    eta.i <- g.mme$eta.i
    v.i <- g.mme$v.i
